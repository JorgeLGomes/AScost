...
if not os.path.isfile('/home/nicole/workdir/SeaWifs/seawifs-clim.1997-2010.nc'):
    #wifs = xr.open_dataset("https://basin.ceoe.udel.edu/thredds/fileServer/SEAWIFSDAILY9KM.nc").sel(lat=slice(5,-55), lon=slice(-69,-9))
    print('Donwload first -> 01')
    #wifs = xr.open_dataset("./SEAWIFSDAILY9KM.nc").sel(lat=slice(5,-55), lon=slice(-69,-9))
    wifs = wifs['chlor_a']
    wifs = wifs.assign_coords({'time':('time',pd.to_datetime(wifs.time.data,format="%Y%m%d"))})
    wifs_mon = wifs.groupby('time.month').mean('time')
    #wifs_mon = wifs.resample(time="1MS").mean('time')
    print(wifs_mon)
...
wifs = xr.open_dataset("/home/nicole/workdir/SeaWifs/seawifs-clim.1997-2010.nc").sel(lat=slice(-20,-40),lon=slice(-60,-20)) #.sel(yq=slice(200,500),xq=slice(200,700))
print(wifs.dims)
wifs.chlor_a.isel(time=0).plot(x='lon',y='lat')
plt.title('Before Regridding - Check domain')
plt.show()
...
seawifs = xr.Dataset({
    'CHL_A': xr.DataArray(
                ...
                dims   = ['time', 'j', 'i'],
                coords = {'time': wifs.time.values, #wifs.month.values,
...
hgrid = xr.open_dataset("/Volumes/A1/workdir/nicole/AScoast/grid/ocean_hgrid.nc")
...
ds.CHL_A.isel(time=0).plot(x='lon',y='lat')
plt.title('After Regridding')
plt.show()

ds.to_netcdf('/home/nicole/workdir/AScoast/INPUT/chl_mom6.nc', 
    encoding=encodings, 
    format='NETCDF4',
    unlimited_dims='time'
)
