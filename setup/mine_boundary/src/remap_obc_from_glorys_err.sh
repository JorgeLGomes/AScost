...
def main():
    ...
    var, ds_grid, depth_vector, ds_var, uv, lati, latf, loni, lonf  = get_args(sys.argv)
    ...
    attrs_dict= { 'zos' :  {    'units' : "meter",      'long_name' : "effective sea level (eta_t + patm/(rho0*g)) on T cells" },
                  'thetao' : {    'units' : "degrees C",  'long_name' : "Potential thetaoerature" },
                  'so' : {    'units' : "psu",        'long_name' : "Practical Salinity" },                  
                    'uo' :  {    'units' : "m s-1",      'long_name' : "Barotropic-step Averaged Zonal Velocity" },
                    'vo' :  {    'units' : "m s-1",      'long_name' : "Barotropic-step Averaged Zonal Velocity" },
...
#    regridder = xe.Regridder(ds_var, da_regrid, "bilinear", periodic=False, reuse_weights=True, loc_stream=True)
#    da_regrid = regridder(ds_var[var])   
    #filling 
...
#    write_obc(ds_out.isel(time=0),var,f"ic_{var}.nc",missval)                    
    ...
    for fnt in fronteiras:
        print('*~ working on obc:', fnt)
        if var == 'zos':
            if fnt == 'north':
                cut_obc=ds_out.isel(lat=slice(-2,None)).mean('lat')
                regridder = xe.Regridder(cut_obc, da_regrid.isel(lat=slice(-2,-1)).mean('lat'), "bilinear", periodic=False, reuse_weights=True, locstream_out=True)
                out_obc = regridder(out_obc[var])
                da_out_obc=out_obc.expand_dims((lat_str),axis=1).assign_coords({lat_str:[lat.data[-1]]}).rename({'lon':lon_str})    

            elif fnt == 'east':
                cut_obc=ds_out.isel(lon=slice(-2,None)).mean('lon')
                regridder = xe.Regridder(cut_obc, da_regrid.isel(lon=slice(-2,-1)).mean('lon'), "bilinear", periodic=False, reuse_weights=True, locstream_out=True)
                out_obc = regridder(out_obc[var])		
                da_out_obc=out_obc.expand_dims((lon_str), axis=2).assign_coords({lon_str: [lon.data[-1]]}).rename({'lat':lat_str})

            elif fnt == 'south':
                cut_obc=ds_out.isel(lat=slice(0,1)).mean('lat')
                regridder = xe.Regridder(cut_obc, da_regrid.isel(lat=slice(0,1)).mean('lat'), "bilinear", periodic=False, reuse_weights=True, locstream_out=True)
                out_obc = regridder(out_obc[var])		
            ...
            if fnt == 'north':
                cut_obc=ds_out.isel(lat=slice(-2,None)).mean('lat')
                dz_obc=da_dz.isel(lat=slice(-2,None)).mean('lat')
                regridder = xe.Regridder(cut_obc, da_regrid.isel(lat=slice(-2,None)).mean('lat'), "bilinear", periodic=False, reuse_weights=True, locstream_out=True)
                out_obc = regridder(out_obc[var])		
                ...
                cut_obc=ds_out.isel(lon=slice(-2,None)).mean('lon')
                dz_obc=da_dz.isel(lon=slice(-2,None)).mean('lon') 
                regridder = xe.Regridder(cut_obc, da_regrid.isel(lon=slice(-2,None)).mean('lon'), "bilinear", periodic=False, reuse_weights=True, locstream_out=True)
                out_obc = regridder(out_obc[var])		
                ...
                cut_obc=ds_out.isel(lat=slice(0,1)).mean('lat').expand_dims(('lat'),axis=2)
                dz_obc=da_dz.isel(lat=slice(0,1)).mean('lat').expand_dims(('lat'),axis=2)
                regridder = xe.Regridder(cut_obc, da_regrid.isel(lat=slice(0,1)).mean('lat').expand_dims(('lat'),axis=2), "bilinear", periodic=False, reuse_weights=True, locstream_out=True)
                out_obc = regridder(cut_obc[var])		
        ...
        if uv:
            ...
            if var == 'uo':
                ...
                print(f'hseg   dz_{var}_{suffix[fnt]}   ', ds_['dz_uo_'+suffix[fnt]].shape, ds_['dz_uo_'+suffix[fnt]].dims)
                ...
                print(f'hqseg  dz_{var}_{suffix[fnt]}   ',ds_['dz_vo_'+suffix[fnt]].shape,ds_['dz_vo_'+suffix[fnt]].dims)
                print(f'hqseg  dz_dvdx_{suffix[fnt]}    ', ds_['dz_dvdx_'+suffix[fnt]].shape,ds_['dz_dvdx_'+suffix[fnt]].dims)
            #print(f'zosseg zeta_{suffix[fnt]}   ',ds_['zos_'+suffix[fnt]].shape)        
            if view_results:
                try:
...
def get_args(args):
    ...
    topog_file=args[3]    
    var=args[4]
    lati, latf, loni, lonf = [float(i) for i in args[5:] ]
    
    ds_grid=xr.open_dataset(static_file)
    depth=xr.open_dataset(topog_file)['depth']    
    
    try:
        ds_var=xr.open_dataset(global_file).sel(longitude=slice(loni,lonf),
    ...
    return var, ds_grid, depth, ds_var, uv, lati, latf, loni, lonf 
#============================================================================= 
...
def fill_obc(ds_,lon=['lon'], lat=['lat']):                        
    ...
    for v in ds_.keys():
        ...
        for coordx in lon:
            try:
                ds_fill[v] = ds_[v].bfill(coordx)
            except:
                ds_fill[v] = ds_[v] 
                pass         
        for coordy in lat:
            try:
                ds_fill[v] = ds_fill[v].ffill(coordy)
            except:
                ds_fill[v] = ds_fill[v] 
                pass
