...
def main():
    ...
    , 'south': {'time':time, 'lev':lev.data, lat_str:[lat.data[0]],  lon_str:lon.data } }
         coord_obc_dz={'north': {'time':time, 'lev':lev.data, lat_str:[lat.data[-1]], lon_str:xh.data  }
    , 'east' : {'time':time, 'lev':lev.data, lat_str: yh.data,      lon_str:[lon.data[-1]]}
    , 'south': {'time':time, 'lev':lev.data, lat_str:[lat.data[0]],  lon_str:xh.data } }                                    
    else:
         coord_obc={'north': {'time':time, lat_str:[lat.data[-1]], lon_str:lon.data}
    ...
    if not os.path.isfile(f'dz_{fnt}_{strvar}.nc') and var not in ['zos','ssh','SSH']:    
	    ...
	    if use_depth_to_limit:
                ...
                if dim == 'lat':
                   dz=np.zeros((len(lev.data),1,len(lon)))
                else:
                   dz=np.zeros((len(lev.data),len(lat),1))
                #dz=[]		
                ...
                #dz = [ [ dzIter(nk, depth_vector[ky,kx], dzTop, Huniform, fnPow,  prec) for ky in range(len(depth_vector[:,0])) ] for kx in range(len(depth_vector[0,:]))]
                depth_vector=depth_vector.isel(sel_slice)
...
#                dz=np.tile(np.array(dz)[np.newaxis,:,:,:],(nt,1,1,1))
                #dz=np.array(dz)
                print(dz.shape)
                dz=np.tile(dz[np.newaxis,:,:,:],(nt,1,1,1))
                if dim == 'lat':
                      dz=np.reshape(dz,(nt,nz, 1, len(lon.data)))
                else:
                      dz=np.reshape(dz,(nt,nz, len(lat.data), 1))
	    ...
	    print(dz.shape)
	    da_dz=xr.DataArray(dz,coords=coord_obc[fnt], name='dz') #.isel(sel_slice)
        ...
        if var in ['uo','u']:
                ...
                if fnt == 'south':
                    ...
                    if supergrid:
                        ...
                        dudy=(da_regrid[:,:,:,-1]-da_regrid[:,:,:,-2]) / dy.values
                        dudy=dudy.expand_dims((lon_str),axis=3).assign_coords({lon_str:[lon.data[-1]]}).rename({'lat':lat_str})
                        ds_['dudy_'+suffix[fnt]] = xr.DataArray( dudy.values, coords={'time':time, 'lev':lev.data, lat_str:lat.data[:], lon_str:[lon.data[-1]]})
                ...
                if fnt == 'south':
                    if supergrid:
                            ...
                            dvdx=(da_regrid[:,:,1,:]-da_regrid[:,:,0,:]) / dx.values
                            dvdx=dvdx.expand_dims((lat_str),axis=2).assign_coords({lat_str:[lat.data[0]]}).rename({'lon':lon_str})
                            ds_['dvdx_'+suffix[fnt]] = xr.DataArray( dvdx.values, coords={'time':time, 'lev':lev.data, lat_str:[lat.data[0]],   lon_str:lon.data[:]})
                elif fnt == 'north': 
                    if supergrid:   
                            ...
                            dvdx=dvdx.expand_dims((lat_str),axis=2).assign_coords({lat_str:[lat.data[-1]]}).rename({'lon':lon_str})
                            ds_['dvdx_'+suffix[fnt]] = xr.DataArray( dvdx.values, coords={'time':time, 'lev':lev.data, lat_str:[lat.data[-1]],   lon_str:lon.data[:]} )
                elif fnt == 'east':
                    dvdx=(da_regrid[:,:,:,-1]-da_regrid[:,:,:,-2]) / dx.values
    ...
    write_obc(ds_out_obc,var,f"obc_{var}_{fnt}.nc",
				fill_value)
...
def get_attrs(var):
     ...
     if var in ['ssh','SSH','zos']:
        ...
        attrs = { "units":'days since 1900-01-01 00:00:00', 
                'calendar' : "gregorian", #'calendar':"365_day",  'modulo' : " ",
                'axis' : "T"}
     elif var in ['title']:                           
...
def segment_defaults(border, nt, lat, lon, nz, lat_str, lon_str):
    ...
    if border == 'north':
       ...
       shape_rgd=(2, len(lon.data))
       coord_rgd={lat_str:[lat.data[-2],lat.data[-1]], lon_str:lon.data}
    elif border == 'south':
       sel_slice=dict(lat=slice(0,1)) ; dim='lat' ; fill=['lon']
       shape_dz=(nt,1, 1, len(lon.data))
       shape_rgd=(2, len(lon.data))
       coord_rgd={ lat_str:[lat.data[0],lat.data[1]], lon_str:lon.data}

    elif border == 'east':
       sel_slice=dict(lon=slice(-2,-1)) ; dim='lon' ; fill=['lat']
       shape_dz=(nt,1, len(lat.data), 1)
       shape_rgd=(len(lat.data), 2)
       coord_rgd={lat_str: lat.data, lon_str:[lon.data[-2],lon.data[-1]]}

    elif border == 'west':
       sel_slice=dict(lon=slice(0,1)) ; dim='lon' ; fill=['lat']
       shape_dz=(nt, 1, len(lat.data), 1)
       shape_rgd=(len(lat.data), 2)
       coord_rgd={lat_str: lat.data, lon_str:[lon.data[0],lon.data[1]]}
...
def get_args(args):
    ...
    depth=xr.open_dataset(topog_file)['depth'].rename({'nx': 'lon', 'ny': 'lat'})
...
#=============================================================================                        
def fill_obc(ds_,dim=['lon','lat'], fill='b'):                        
