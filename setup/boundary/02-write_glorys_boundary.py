...
def write_year(year, glorys_dir, segments, is_first_year=False):
    glorys = (
        xarray.open_mfdataset(sorted(glob(os.path.join(glorys_dir, f'{year}/glorys_{year}1*.nc'))))
...
def main():
    ...
    output_dir = '/home/nicole/workdir/AScoast/boundary/'
    ncrcat_outdir = '/home/nicole/workdir/AScoast/boundary_final/'
    hgrid = xarray.open_dataset('/home/nicole/workdir/AScoast/grid/ocean_hgrid.nc')

#    vgrid_file = '/Volumes/A1/workdir/nicole/AScoast/vgrid/vgrid_75_2m.nc' #(new!)
...
#    )
    ...
    if len(sys.argv)<=1:
        print(f'write_year_indiv=False')
        write_year_indiv=False
    else:
        write_year_indiv=sys.argv[1]

    for y in range(2001, 2003):
        print(y)
        if write_year_indiv or write_year_indiv == 1:
